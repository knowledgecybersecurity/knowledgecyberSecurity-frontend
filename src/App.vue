<template>
  <v-app>
    <v-app-bar app color="#42b883" dark>
      <v-app-bar-title> Knowledge CyberSecurity </v-app-bar-title>
      <v-spacer></v-spacer>
      <div class="links">
        ...
        <v-col cols="12">
          <v-menu offset-y>
            <template v-slot:activator="{ on, attrs }">
              <v-btn color="teal lighten-2" dark v-bind="attrs" v-on="on">
                <!-- {{ getUserEmail() }} -->
            ...
            <v-list>
              <v-list-item
                ...
                :key="index"
              >
                <v-list-item-title @click="selectProfileAction(item)">{{
                  item.title
                }}</v-list-item-title>
              </v-list-item>
            </v-list>
...
export default {
  ...
  data: () => ({
    items: [
      ...
      { title: "My publications", action: "myPublications" },
    ],
  }),
  methods: {
    isAdmin() {
    ...
    selectProfileAction(item) {
      ...
      switch (item.action) {
        ...
        case "favorites":
          ...
          break;
        case "myPublications":
          this.$router.push(`${"/myPublications/" + user.id}`);
          break;
        case "logout":
          this.logout();
          break;
