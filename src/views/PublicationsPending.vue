              ...
              <v-col cols="12">
                <v-card v-for="(item, index) in publications" :key="index" class="mx-auto" max-width="auto" outlined>
                  ...
                  <v-card-actions>
                    ...
                    <v-btn @click="aprobe(item.id)" outlined rounded text> Aprobe </v-btn>
                  </v-card-actions>
                </v-card>
...
export default {
  ...
  methods: {
    ...
    getPublications(page) {
      ...
      axios.get(`${BASE_URL}/publications/listPaginationNotAprovved/${page}/5`)
        .then((response) =>{
    ...
    aprobe(id) {
        axios.get(`${BASE_URL}/publications/aprobe/${id}`)
        .then((response) =>{
